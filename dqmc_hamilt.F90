...
implicit none
 ...
 type :: Hamiltonian_t
   ...
   integer, pointer :: rt(:,:), lf(:,:), up(:,:), dn(:,:)
   integer, pointer       :: rt1(:), lf1(:), up1(:), dn1(:)
   integer, pointer       :: uprt(:), dnlf(:), uplf(:), dnrt(:)
...
!-----------------------------------------------------------------!
 ...
 subroutine free_hamilt(hamilt)
   ...
   !
    ...
    if(associated(hamilt%rt))         deallocate(hamilt%rt)
    if(associated(hamilt%lf))         deallocate(hamilt%lf)
    if(associated(hamilt%up))         deallocate(hamilt%up)
    if(associated(hamilt%dn))         deallocate(hamilt%dn)
...
!-----------------------------------------------------------------!
 ...
 subroutine construct_hamilt(hamilt, lattice, recip_lattice, cfg)
   ...
   !
    ...
    real*8  :: x, y, z, xmax, ymax, zmax,dxmax, dymax, dzmax, tmp
   ...
   !  For 3band model dd d-wave susceptibility
    allocate(hamilt%rt(0:nsites-1, 3))
    allocate(hamilt%lf(0:nsites-1, 3))
    allocate(hamilt%up(0:nsites-1, 3))
    allocate(hamilt%dn(0:nsites-1, 3))
   !  For 3band model dp d-wave susceptibility
    allocate(hamilt%rt1(0:nsites-1))
    allocate(hamilt%lf1(0:nsites-1))
    allocate(hamilt%up1(0:nsites-1))
    allocate(hamilt%dn1(0:nsites-1))

    allocate(hamilt%uprt(0:nsites-1))
    allocate(hamilt%uplf(0:nsites-1))
    allocate(hamilt%dnrt(0:nsites-1))
    allocate(hamilt%dnlf(0:nsites-1))
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !2025 05 02: Below for the real xmax. For 3band model and N=4, xmax=3.5
    xmax = maxval(pos(1,:))
    ymax = maxval(pos(2,:))
    zmax = maxval(pos(3,:))
    !2025 05 02: Below for 1band or 3band model to find neighbors
    !additonal mask to make sure dxmax is the largest d orb position(not px or py)
    !For 3band model and N=4, dxmax=3.0
    dxmax = maxval( pos(1,:), mask = abs(pos(1,:) - int(pos(1,:))) < 1.0e-6 )
    dymax = maxval( pos(2,:), mask = abs(pos(2,:) - int(pos(2,:))) < 1.0e-6 )
    dzmax = maxval( pos(3,:), mask = abs(pos(3,:) - int(pos(3,:))) < 1.0e-6 )
    ...
    do it = 0, nsites-1
      if (mod(it, 3)==0) then                                                                                                        
         ...
         tmp = plus(x,dxmax, 1.0d0)  
         hamilt%rt(it, 1) = pos2site(tmp,y,z)
         hamilt%rt(it, 2) = pos2site(tmp,y,z)+1
         hamilt%rt(it, 3) = pos2site(tmp,y,z)+2
         tmp = minus(x,dxmax, 1.0d0)
         hamilt%lf(it, 1) = pos2site(tmp,y,z)
         hamilt%lf(it, 2) = pos2site(tmp,y,z)+1
         hamilt%lf(it, 3) = pos2site(tmp,y,z)+2
         tmp = plus(y,dymax, 1.0d0)
         hamilt%up(it, 1) = pos2site(x,tmp,z)
         hamilt%up(it, 2) = pos2site(x,tmp,z)+1
         hamilt%up(it, 3) = pos2site(x,tmp,z)+2
         tmp = minus(y,dymax, 1.0d0)
         hamilt%dn(it, 1) = pos2site(x,tmp,z)
         hamilt%dn(it, 2) = pos2site(x,tmp,z)+1
         hamilt%dn(it, 3) = pos2site(x,tmp,z)+2
      endif
    enddo
    ...
    do it = 0, nsites-1
      if (mod(it, 3)==0) then                                                                                                        
         ...
         tmp = plus(x,xmax, 0.5d0)  
         hamilt%rt1(it) = pos2site(tmp,y,z)
         tmp = minus(x,xmax, 0.5d0)
         hamilt%lf1(it) = pos2site(tmp,y,z)
         tmp = plus(y,ymax, 0.5d0)
         hamilt%up1(it) = pos2site(x,tmp,z)
         tmp = minus(y,ymax, 0.5d0)
         hamilt%dn1(it) = pos2site(x,tmp,z)
      endif
    enddo
...
!        do is = 0, nsites-1
!          write(*,*) is, "hopup to:", hamilt%hopup(is,:)
!        enddo
    ...
    contains
       ...
       function plus(i,imax, itv) result(ip)
         real*8, intent(in) :: i, imax, itv ! input
         real*8             :: ip      ! output
         ip = i+itv
         if (ip>imax) then
           ip = 0.0
       ...
       function minus(i,imax, itv) result(im)
         real*8, intent(in) :: i, imax, itv ! input
         real*8             :: im      ! output
         im = i-itv
         if (im<0) then
       ...
       function pos2site(x, y, z) result(isite)
         implicit none
         ! Input
         ...
         ! Locals
         integer :: it
         real, parameter :: tol = 1.0e-5  ! tolerance for floating point comparison

         pos => lattice%cartpos

         isite = -1  ! default: not found

         do it = 0, nsites-1
            if (abs(pos(1,it) - x) < tol .and. &
                  abs(pos(2,it) - y) < tol .and. &
                  abs(pos(3,it) - z) < tol) then
                  isite = it
                  return
            end if
         end do

       end function pos2site
