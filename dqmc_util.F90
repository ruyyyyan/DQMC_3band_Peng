...
!  use BLAS_MOD
  ...
  interface DQMC_Print_Array
     module procedure DQMC_Print_RealArray, DQMC_Print_RealArrayMJ, &
                      DQMC_Print_RealArrayMJ1, DQMC_Print_ComplexArray
...
!    enddo
 ...
 !--------------------------------------------------------!
  ...
  subroutine DQMC_Print_RealArrayMJ(n, m, label, avg, err, OPT)
    !
    ! Purpose
    ! =======
    !    This subroutine prints out the content of avg +- err
  ...
  end subroutine DQMC_Print_RealArrayMJ

  !--------------------------------------------------------------------!

  subroutine DQMC_Print_RealArrayMJ1(m, label, avg, err, OPT)
    !
    ! Purpose
    ! =======
    !    This subroutine prints out the content of avg +- err
    !    ONLY prints out the value at tau = beta/2
    !
    ! Arguments
    ! =========
    integer,  intent(in)    :: OPT, m
    character(*), intent(in):: label(:)
    real(wp), intent(in)    :: avg(:,:), err(:,:)
   ...
   do j = m/2+1, m/2+1
  !    write(OPT, "(a,e16.8, e16.8)") label(j), avg(j,1), err(j,1)
      write(OPT, "(e16.8)") avg(j,1)
   end do

  end subroutine DQMC_Print_RealArrayMJ1
...
!               label(j), dble(avg(j,1)), dble(err(j,1)), aimag(avg(j,1)), aimag(err(j,1))
  ...
  subroutine dqmc_GetFTk(value, n, nclass, class, na, nk, ft_wgt, phase, valuek)
    ...
    ! na means natom in unit cell
     ...
     real(wp)    :: alpha1  ! Filter strength parameter
           ...
           do ja = ia, na
              ...
              do it = 1, nt
                 do jt = 1, nt
                    ...
                    i = (it-1) * na + ia
                    j = (jt-1) * na + ja
                    filter_wgt = exp(-1 * alpha1 * sqrt(real(it * it + jt * jt, wp)))
                    filter_wgt = 1.0
                    ! Use class that corresponds to the (i, j) pair
                  !   write(*,*) "phase(", i, ",", j, ") =", phase(i, j)
 
                    phcurr = phase(i, j) * value(class(i, j)) * filter_wgt
  ...
  subroutine convert_to_iw0_real(tdmtau, tdmiw0, L, dtau)
     ! 3/28/2019:
     ! convert G(tau) or spin-spin(tau) to G(iw=0) or spin(iw=0) component
     ! using composite Simpson's rule

     integer,  intent(in)  :: L
     real(wp), intent(in)  :: tdmtau(0:L-1)
  ...
  end subroutine convert_to_iw0_real
...
1       IF (KHI-KLO.GT.1) THEN
  ...
  subroutine DQMC_IO_open(fname, INP_UNIT, OUT_UNIT)
    ...
    ! =========
     ...
     character(len=50), intent(out) :: fname
