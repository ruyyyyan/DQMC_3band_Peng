...
#include "dqmc_include.h"
 ...
 ! use BLAS_MOD
  ...
  type TDM
     ...
     ! (nClass, T1%err)
     real(wp), pointer :: chirxx_iw0(:,:)  
     real(wp), pointer :: chirzz_iw0(:,:)
     ...
     ! 250508 In 3band Hubbard model, rt lf up and dn ,each direction has a unit cell which contains 3 atoms
 ...
 !--------------------------------------------------------------------!
  ...
  subroutine DQMC_TDM_Init(model, L, dtau, T1, nBin, S, Gwrap, flags, flagsFT)!, splinew0)
    ...
    if (T1%flags(ISPXX) == 1) then
      ...
      allocate(T1%chirxx_iw0(nClass, T1%err))
      T1%spinxxAvg = 0.0_wp
    endif
    ...
    if (T1%flags(ISPZZ) == 1) then
      ...
      allocate(T1%chirzz_iw0(nClass, T1%err))
      T1%spinzzAvg = 0.0_wp
    endif
    ...
    if (T1%flags(IPAIRd) == 1) then
      ...
      select case (model)
        ! hubbard square
        ! 250529 compute 9 components in isotroic 3band Hubbard model:
        ! 1       2    3     4      5      6       7          8      9        (index)
        ! P_total dddd dddpx ddpxpx dpxdpx dpxpxpx pxpxpxpx | ddpxpy pypypypy 
        ! 81      1    4     2      4      4       1          4      1        (multiplicity)
        case (0)
          T1%NPd = 9
    ...
    allocate(T1%rt(1:S%nSite, 3))
    allocate(T1%lf(1:S%nSite, 3))
    allocate(T1%up(1:S%nSite, 3))
    allocate(T1%dn(1:S%nSite, 3))

    allocate(T1%hopup(1:S%nSite,1:S%nSite))
    allocate(T1%hopdn(1:S%nSite,1:S%nSite))

    do i = 1, S%nSite
      !250514 For 3band model, one unit cell contains 3 atoms. 
      do ii = 1, 3
       T1%rt(i, ii) = Gwrap%Hamilt%rt(i-1, ii)+1
       T1%lf(i, ii) = Gwrap%Hamilt%lf(i-1, ii)+1
       T1%up(i, ii) = Gwrap%Hamilt%up(i-1, ii)+1
       T1%dn(i, ii) = Gwrap%Hamilt%dn(i-1, ii)+1
      enddo
...
!write(*,*) "a2"
       ...
       if (T1%flags(iprop)==1) then       
          ...
          do i = 1, T1%properties(iprop)%nClass
            ...
            if (iprop==ISPXX) then
              ...
              T1%chirxx_iw0(i,T1%idx) = T1%chirxx_iw0(i,T1%idx) + factor*chiv  
            endif

            if (iprop==ISPZZ) then
              chiv = 0.0
              call convert_to_iw0_real(values(i,0:L-1,T1%tmp), chiv, L, T1%dtau)
              T1%chirzz_iw0(i,T1%idx) = T1%chirzz_iw0(i,T1%idx) + factor*chiv  
            endif
          end do
          values(:,:,T1%tmp) = ZERO
       endif
    ...
    ! compute chixx_q at q=(0,0) and (pi,pi)
    if (T1%flagsFT(ISPXX) == 1) then
      factor = sgn/cnt
    ...
    if (T1%flags(IPAIRd) == 1) then
      ...
      ! write(*,*) 'Pd(tau=0) idx =', T1%idx, ':', T1%Pdtau(0, T1%idx, 1)
      ...
      ! write(*,*) 'Pd idx =', T1%idx, ':', T1%Pd(T1%idx, 1)
  ...
  subroutine DQMC_TDM_Compute(T1, model, upt0, up0t, dnt0, dn0t, up00, uptt, dn00, dntt, it, i0)
    ...
    integer  :: i, j, k, dt, dt1, dt2, b1, b2, idx, o2, L, al1, al2, al3, al4, ire, jre
          ...
          do j = 1,  T1%properties(ISPXX)%n
           ...
           !       + up0t(j,i)*dnt0(i,j))/2
             ...
             if (T1%flagsFT(ISPXX) == 1) then
               ...
               b1 = int(vec(1))+1        ! +1 because b1,b2 starts from 0
               b2 = int(vec(2))+1
               idx = (b1-1)*T1%norb+b2
          ...
          do j = 1, T1%properties(ISPZZ)%n
            ! k is the distance index of site i and site j
             ...
             if (T1%flagsFT(ISPZZ) == 1) then
               ...
               vec = T1%properties(ISPZZ)%vecClass(k,:)
               b1 = int(vec(1))+1        ! +1 because b1,b2 starts from 0
               b2 = int(vec(2))+1
               idx = (b1-1)*T1%norb+b2
     ...
     if (T1%flags(IPAIRs) == 1) then
       ...
       if (model==4) then
           do i = 1,  T1%properties(IPAIRs)%n
              ...
              !   if (upt0(i,j)*dnt0(i+1,j+1)>0.1) then
              !     write(*,*) 'dt!=0, upt0(i,j),dnt0(i+1,j+1)=', upt0(i,j), dnt0(i+1,j+1), upt0(i,j)*dnt0(i+1,j+1), i, j
              !   endif
       ...
       do i = 1,  T1%properties(IPAIRd)%n
          do j = 1,  T1%properties(IPAIRd)%n
              ...
              !   endif
                ...
                ! 250515  for 3band model tensor P has 3^4 = 81 components.
                ...
                if (mod(i-1,3)==0 .and. mod(j-1,3)==0) then
                  do al2 = 1, 3
                    do al3 = 1, 3
                      ire = i+al2-1
                      jre = j+al3-1
                      k = T1%properties(IPAIRd)%D(ire,jre)
                      vec = T1%properties(IPAIRd)%vecClass(k,:)
                      ! Here P has total 81 terms, al1 to al4 denote 4 indexs under P. 
                      do al1 = 1, 3
                        do al4 = 1, 3
                          a =  dnt0(T1%rt(i, al4), T1%rt(j, al1)) - dnt0(T1%rt(i, al4), T1%up(j, al1))  &
                              +dnt0(T1%rt(i, al4), T1%lf(j, al1)) - dnt0(T1%rt(i, al4), T1%dn(j, al1))  &
                              -dnt0(T1%up(i, al4), T1%rt(j, al1)) + dnt0(T1%up(i, al4), T1%up(j, al1))  &
                              -dnt0(T1%up(i, al4), T1%lf(j, al1)) + dnt0(T1%up(i, al4), T1%dn(j, al1))  &
                              +dnt0(T1%lf(i, al4), T1%rt(j, al1)) - dnt0(T1%lf(i, al4), T1%up(j, al1))  &
                              +dnt0(T1%lf(i, al4), T1%lf(j, al1)) - dnt0(T1%lf(i, al4), T1%dn(j, al1))  &
                              -dnt0(T1%dn(i, al4), T1%rt(j, al1)) + dnt0(T1%dn(i, al4), T1%up(j, al1))  &
                              -dnt0(T1%dn(i, al4), T1%lf(j, al1)) + dnt0(T1%dn(i, al4), T1%dn(j, al1))  

                          b =  dn0t(T1%rt(i, al4), T1%rt(j, al1)) - dn0t(T1%rt(i, al4), T1%up(j, al1))  &
                              +dn0t(T1%rt(i, al4), T1%lf(j, al1)) - dn0t(T1%rt(i, al4), T1%dn(j, al1))  &
                              -dn0t(T1%up(i, al4), T1%rt(j, al1)) + dn0t(T1%up(i, al4), T1%up(j, al1))  &
                              -dn0t(T1%up(i, al4), T1%lf(j, al1)) + dn0t(T1%up(i, al4), T1%dn(j, al1))  &
                              +dn0t(T1%lf(i, al4), T1%rt(j, al1)) - dn0t(T1%lf(i, al4), T1%up(j, al1))  &
                              +dn0t(T1%lf(i, al4), T1%lf(j, al1)) - dn0t(T1%lf(i, al4), T1%dn(j, al1))  &
                              -dn0t(T1%dn(i, al4), T1%rt(j, al1)) + dn0t(T1%dn(i, al4), T1%up(j, al1))  &
                              -dn0t(T1%dn(i, al4), T1%lf(j, al1)) + dn0t(T1%dn(i, al4), T1%dn(j, al1))

                          ! *0.25 or /4 accounts for the convention for definition
                          ! See 1989 PRB paper: Numerical study of 2D Hubbard model
                          a = a*0.5_wp*0.25
                          b = b*0.5_wp*0.25
                          value1(k)  = value1(k) + upt0(ire,jre)*a
                          value2(k)  = value2(k) + up0t(ire,jre)*b 

                          ! P_total
                          T1%Pdtau(dt1, T1%tmp, 1) = T1%Pdtau(dt1, T1%tmp, 1) + upt0(ire,jre)*a
                          T1%Pdtau(dt2, T1%tmp, 1) = T1%Pdtau(dt2, T1%tmp, 1) + up0t(ire,jre)*b 
                          ! P_dddd
                          if (al1==1 .and. al2==1 .and. al3==1 .and. al4==1) then
                            T1%Pdtau(dt1, T1%tmp, 2) = T1%Pdtau(dt1, T1%tmp, 2) + upt0(ire,jre)*a
                            T1%Pdtau(dt2, T1%tmp, 2) = T1%Pdtau(dt2, T1%tmp, 2) + up0t(ire,jre)*b 
                          elseif (count((/al1, al2, al3, al4/) == 1) == 3 .and. count((/al1, al2, al3, al4/) == 2) == 1) then

                            T1%Pdtau(dt1, T1%tmp, 3) = T1%Pdtau(dt1, T1%tmp, 3) + upt0(ire,jre)*a
                            T1%Pdtau(dt2, T1%tmp, 3) = T1%Pdtau(dt2, T1%tmp, 3) + up0t(ire,jre)*b
                          elseif (count((/al1, al2, al3, al4/) == 1) == 2 .and. count((/al1, al2, al3, al4/) == 2) == 2 .and. mod(al1+al2, 2) == 0) then
                            T1%Pdtau(dt1, T1%tmp, 4) = T1%Pdtau(dt1, T1%tmp, 4) + upt0(ire,jre)*a
                            T1%Pdtau(dt2, T1%tmp, 4) = T1%Pdtau(dt2, T1%tmp, 4) + up0t(ire,jre)*b
                          elseif (count((/al1, al2, al3, al4/) == 1) == 2 .and. count((/al1, al2, al3, al4/) == 2) == 2 .and. mod(al1+al2, 2) == 1) then
                            T1%Pdtau(dt1, T1%tmp, 5) = T1%Pdtau(dt1, T1%tmp, 5) + upt0(ire,jre)*a
                            T1%Pdtau(dt2, T1%tmp, 5) = T1%Pdtau(dt2, T1%tmp, 5) + up0t(ire,jre)*b
                          elseif (count((/al1, al2, al3, al4/) == 1) == 1 .and. count((/al1, al2, al3, al4/) == 2) == 3) then
                            T1%Pdtau(dt1, T1%tmp, 6) = T1%Pdtau(dt1, T1%tmp, 6) + upt0(ire,jre)*a
                            T1%Pdtau(dt2, T1%tmp, 6) = T1%Pdtau(dt2, T1%tmp, 6) + up0t(ire,jre)*b
                          elseif ((al1==2) .and. (al2==2) .and. (al3==2) .and. (al4==2)) then
                            T1%Pdtau(dt1, T1%tmp, 7) = T1%Pdtau(dt1, T1%tmp, 7) + upt0(ire,jre)*a
                            T1%Pdtau(dt2, T1%tmp, 7) = T1%Pdtau(dt2, T1%tmp, 7) + up0t(ire,jre)*b
                          elseif (count((/al1, al2, al3, al4/) == 2) == 1 .and. count((/al1, al2, al3, al4/) == 3) == 1 .and. &
                                  ((al1+al2)==2 .or. (al3+al4==2))) then
                            T1%Pdtau(dt1, T1%tmp, 8) = T1%Pdtau(dt1, T1%tmp, 8) + upt0(ire,jre)*a
                            T1%Pdtau(dt2, T1%tmp, 8) = T1%Pdtau(dt2, T1%tmp, 8) + up0t(ire,jre)*b
                          elseif ((al1==3) .and. (al2==3) .and. (al3==3) .and. (al4==3)) then
                            T1%Pdtau(dt1, T1%tmp, 9) = T1%Pdtau(dt1, T1%tmp, 9) + upt0(ire,jre)*a
                            T1%Pdtau(dt2, T1%tmp, 9) = T1%Pdtau(dt2, T1%tmp, 9) + up0t(ire,jre)*b
                          endif
     ...
     if (T1%flags(ICOND) == 1) then
       ...
       do i = 1,  T1%properties(ICOND)%n
          do j = 1,  T1%properties(ICOND)%n
            ...
            a = T1%hopup(i,T1%rt(i, 1))*T1%hopup(j,T1%rt(j, 1))
            c = T1%hopup(i,T1%rt(i, 1))*T1%hopdn(j,T1%rt(j, 1))

            ! up*up terms (note two ways for contraction!)           
            value1(k) = value1(k) - a* &
                        (-upt0(i,T1%rt(j, 1))*up0t(j,T1%rt(i, 1)) - upt0(T1%rt(i, 1),j)*up0t(T1%rt(j, 1),i) &
                         +upt0(i,j)*up0t(T1%rt(j, 1),T1%rt(i, 1)) + upt0(T1%rt(i, 1),T1%rt(j, 1))*up0t(j,i))*0.5_wp
            value1(k) = value1(k) - a* &
                        (uptt(i,T1%rt(i, 1)) - uptt(T1%rt(i, 1),i))*(up00(j,T1%rt(j, 1)) - up00(T1%rt(j, 1),j))*0.5_wp
            ! up*dn terms
            value1(k) = value1(k) - c* &
                        (uptt(i,T1%rt(i, 1)) - uptt(T1%rt(i, 1),i))*(dn00(j,T1%rt(j, 1)) - dn00(T1%rt(j, 1),j))*0.5_wp

            ! Below assuming isotropic system
            ! for averaging over x and y directions
            ! for anisotropic systems, need to modify this part
            ! a = T1%hopup(i,T1%up(i, 1))*T1%hopup(j,T1%up(j, 1))
            ! c = T1%hopup(i,T1%up(i, 1))*T1%hopdn(j,T1%up(j, 1))

            ! up*up terms (note two ways for contraction!)
            !  value1(k) = value1(k) - a* &
            !              (-upt0(i,T1%up(j, 1))*up0t(j,T1%up(i, 1)) - upt0(T1%up(i, 1),j)*up0t(T1%up(j, 1),i) &
            !               +upt0(i,j)*up0t(T1%up(j, 1),T1%up(i, 1)) + upt0(T1%up(i, 1),T1%up(j, 1))*up0t(j,i))*0.5_wp
            !  value1(k) = value1(k) - a* &
            !              ( uptt(i,T1%up(i, 1))*up00(j,T1%up(j, 1)) - uptt(T1%up(i, 1),i)*up00(T1%up(j, 1),j) &
            !               -uptt(i,T1%up(i, 1))*up00(T1%up(j, 1),j) + uptt(T1%up(i, 1),i)*up00(j,T1%up(j, 1)))*0.5_wp
            !   up*dn terms
            !  value1(k) = value1(k) - c* &
            !              ( uptt(i,T1%up(i, 1))*dn00(j,T1%up(j, 1)) + uptt(T1%up(i, 1),i)*dn00(T1%up(j, 1),j) &
            !               -uptt(i,T1%up(i, 1))*dn00(T1%up(j, 1),j) - uptt(T1%up(i, 1),i)*dn00(j,T1%up(j, 1)))*0.5_wp
  
            ! value2: see the rules at the beginning of routine
            a = T1%hopup(j,T1%rt(j, 1))*T1%hopup(i,T1%rt(i, 1))
            c = T1%hopup(j,T1%rt(j, 1))*T1%hopdn(i,T1%rt(i, 1))
            ! up*up terms
            value2(k) = value2(k) - a* &
                        (-upt0(j,T1%rt(i, 1))*up0t(i,T1%rt(j, 1)) - upt0(T1%rt(j, 1),i)*up0t(T1%rt(i, 1),j) &
                         +upt0(j,i)*up0t(T1%rt(i, 1),T1%rt(j, 1)) + upt0(T1%rt(j, 1),T1%rt(i, 1))*up0t(i,j))*0.5_wp
            value2(k) = value2(k) - a* &
                        (uptt(j,T1%rt(j, 1)) - uptt(T1%rt(j, 1),j))*(up00(i,T1%rt(i, 1)) - up00(T1%rt(i, 1),i))*0.5_wp
            ! up*dn terms
            value2(k) = value2(k) - c* &
                        (uptt(j,T1%rt(j, 1)) - uptt(T1%rt(j, 1),j))*(dn00(i,T1%rt(i, 1)) - dn00(T1%rt(i, 1),i))*0.5_wp

            ! a = T1%hopup(j,T1%up(j, 1))*T1%hopup(i,T1%up(i, 1))
            ! c = T1%hopup(j,T1%up(j, 1))*T1%hopdn(i,T1%up(i, 1))
            ! up*up terms
            ! value2(k) = value2(k) - a* &
            !             (-upt0(j,T1%up(i, 1))*up0t(i,T1%up(j, 1)) - upt0(T1%up(j, 1),i)*up0t(T1%up(i, 1),j) &
            !               +upt0(j,i)*up0t(T1%up(i, 1),T1%up(j, 1)) + upt0(T1%up(j, 1),T1%up(i, 1))*up0t(i,j))*0.5_wp
            ! value2(k) = value2(k) - a* &
            !             ( uptt(j,T1%up(j, 1))*up00(i,T1%up(i, 1)) - uptt(T1%up(j, 1),j)*up00(T1%up(i, 1),i) &
            !               -uptt(j,T1%up(j, 1))*up00(T1%up(i, 1),i) + uptt(T1%up(j, 1),j)*up00(i,T1%up(i, 1)))*0.5_wp
            ! up*dn terms
            ! value2(k) = value2(k) - c* &
            !             ( uptt(j,T1%up(j, 1))*dn00(i,T1%up(i, 1)) + uptt(T1%up(j, 1),j)*dn00(T1%up(i, 1),i) &
            !               -uptt(j,T1%up(j, 1))*dn00(T1%up(i, 1),i) - uptt(T1%up(j, 1),j)*dn00(i,T1%up(i, 1)))*0.5_wp
          end do
       end do
       ...
       do i = 1,  T1%properties(ICOND)%n
          do j = 1,  T1%properties(ICOND)%n
            ...
            b = T1%hopdn(i,T1%rt(i, 1))*T1%hopdn(j,T1%rt(j, 1))
            d = T1%hopdn(i,T1%rt(i, 1))*T1%hopup(j,T1%rt(j, 1))

            ! dn*dn terms (note two ways for contraction!)
            value1(k) = value1(k) - b* &
                        (-dnt0(i,T1%rt(j, 1))*dn0t(j,T1%rt(i, 1)) - dnt0(T1%rt(i, 1),j)*dn0t(T1%rt(j, 1),i) &
                         +dnt0(i,j)*dn0t(T1%rt(j, 1),T1%rt(i, 1)) + dnt0(T1%rt(i, 1),T1%rt(j, 1))*dn0t(j,i))*0.5_wp
            value1(k) = value1(k) - b* &
                        (dntt(i,T1%rt(i, 1)) - dntt(T1%rt(i, 1),i))*(dn00(j,T1%rt(j, 1)) - dn00(T1%rt(j, 1),j))*0.5_wp
            ! dn*up terms
            value1(k) = value1(k) - d* &
                        (dntt(i,T1%rt(i, 1)) - dntt(T1%rt(i, 1),i))*(up00(j,T1%rt(j, 1)) - up00(T1%rt(j, 1),j))*0.5_wp

            ! Below assuming isotropic system
            ! for averaging over x and y directions
            ! for anisotropic systems, need to modify this part
            ! b = T1%hopdn(i,T1%up(i, 1))*T1%hopdn(j,T1%up(j, 1))
            ! d = T1%hopdn(i,T1%up(i, 1))*T1%hopup(j,T1%up(j, 1))

            ! dn*dn terms (note two ways for contraction!)
            ! value1(k) = value1(k) - b* &
            !             (-dnt0(i,T1%up(j, 1))*dn0t(j,T1%up(i, 1)) - dnt0(T1%up(i, 1),j)*dn0t(T1%up(j, 1),i) &
            !               +dnt0(i,j)*dn0t(T1%up(j, 1),T1%up(i, 1)) + dnt0(T1%up(i, 1),T1%up(j, 1))*dn0t(j,i))*0.5_wp
            ! value1(k) = value1(k) - b* &
            !             ( dntt(i,T1%up(i, 1))*dn00(j,T1%up(j, 1)) - dntt(T1%up(i, 1),i)*dn00(T1%up(j, 1),j) &
            !               -dntt(i,T1%up(i, 1))*dn00(T1%up(j, 1),j) + dntt(T1%up(i, 1),i)*dn00(j,T1%up(j, 1)))*0.5_wp
            ! dn*up terms
            ! value1(k) = value1(k) - d* &
            !             ( dntt(i,T1%up(i, 1))*up00(j,T1%up(j, 1)) + dntt(T1%up(i, 1),i)*up00(T1%up(j, 1),j) &
            !               -dntt(i,T1%up(i, 1))*up00(T1%up(j, 1),j) - dntt(T1%up(i, 1),i)*up00(j,T1%up(j, 1)))*0.5_wp

            ! value2: see the rules at the beginning of routine
            b = T1%hopdn(j,T1%rt(j, 1))*T1%hopdn(i,T1%rt(i, 1))
            d = T1%hopdn(j,T1%rt(j, 1))*T1%hopup(i,T1%rt(i, 1))

            ! dn*dn terms (note two ways for contraction!)
            value2(k) = value2(k) - b* &
                        (-dnt0(j,T1%rt(i, 1))*dn0t(i,T1%rt(j, 1)) - dnt0(T1%rt(j, 1),i)*dn0t(T1%rt(i, 1),j) &
                         +dnt0(j,i)*dn0t(T1%rt(i, 1),T1%rt(j, 1)) + dnt0(T1%rt(j, 1),T1%rt(i, 1))*dn0t(i,j))*0.5_wp
            value2(k) = value2(k) - b* &
                        (dntt(j,T1%rt(j, 1)) - dntt(T1%rt(j, 1),j))*(dn00(i,T1%rt(i, 1)) - dn00(T1%rt(i, 1),i))*0.5_wp
            ! dn*up terms
            value2(k) = value2(k) - d* &
                        (dntt(j,T1%rt(j, 1)) - dntt(T1%rt(j, 1),j))*(up00(i,T1%rt(i, 1)) - up00(T1%rt(i, 1),i))*0.5_wp

            ! for averaging over x and y directions
            ! b = T1%hopdn(j,T1%up(j, 1))*T1%hopdn(i,T1%up(i, 1))
            ! d = T1%hopdn(j,T1%up(j, 1))*T1%hopup(i,T1%up(i, 1))

            ! dn*dn terms (note two ways for contraction!)
            ! value2(k) = value2(k) - b* &
            !             (-dnt0(j,T1%up(i, 1))*dn0t(i,T1%up(j, 1)) - dnt0(T1%up(j, 1),i)*dn0t(T1%up(i, 1),j) &
            !               +dnt0(j,i)*dn0t(T1%up(i, 1),T1%up(j, 1)) + dnt0(T1%up(j, 1),T1%up(i, 1))*dn0t(i,j))*0.5_wp
            ! value2(k) = value2(k) - b* &
            !             ( dntt(j,T1%up(j, 1))*dn00(i,T1%up(i, 1)) - dntt(T1%up(j, 1),j)*dn00(T1%up(i, 1),i) &
            !               -dntt(j,T1%up(j, 1))*dn00(T1%up(i, 1),i) + dntt(T1%up(j, 1),j)*dn00(i,T1%up(i, 1)))*0.5_wp
            ! dn*up terms
            ! value2(k) = value2(k) - d* &
            !             ( dntt(j,T1%up(j, 1))*up00(i,T1%up(i, 1)) + dntt(T1%up(j, 1),j)*up00(T1%up(i, 1),i) &
            !               -dntt(j,T1%up(j, 1))*up00(T1%up(i, 1),i) - dntt(T1%up(j, 1),j)*up00(i,T1%up(i, 1)))*0.5_wp
          end do
       end do
              ...
              do j = 1,  T1%properties(IPAIRs)%n
                ...
                ! endif
                 ...
                 if ( abs(z1-1.d0)<1.d-6 .and. abs(z2-1.d0)<1.d-6) then
                  ...
                  !   if (val1*val2>0.1) then
                  !     write(*,*) 'dt==0, val1,val2=', val1, val2, val1*val2, i, j
                  !   endif
                     val3 = val1*val2
            ...
            if (mod(i-1,3)==0 .and. mod(j-1,3)==0) then
              do al2 = 1,   3
                do al3 = 1,   3
                  ...
                  k = T1%properties(IPAIRd)%D(ire,jre)
                  vec = T1%properties(IPAIRd)%vecClass(k,:)
                  ! Here P has total 81 terms but only 6 type inequivalent components: dddd, dddp, ddpp, dpdp, dppp and pppp for isotropic 3band model.
                  ! The multiplicity of the same type of component:                    1,    8,    8,    16,   32,      16
                  do al1 = 1,   3
                    do al4 = 1,   3
                      a =  dnt0(T1%rt(i, al4), T1%rt(j, al1)) - dnt0(T1%rt(i, al4), T1%up(j, al1))  &
                          +dnt0(T1%rt(i, al4), T1%lf(j, al1)) - dnt0(T1%rt(i, al4), T1%dn(j, al1))  &
                      ...
                      ! see Ben's notes
                      val1 = one_G(up00(ire,jre), ire, jre)
                      val2 =  one_G(dn00(T1%rt(i, al4), T1%rt(j, al1)), T1%rt(i, al4), T1%rt(j, al1))  &
                            - one_G(dn00(T1%rt(i, al4), T1%up(j, al1)), T1%rt(i, al4), T1%up(j, al1))  &
                            + one_G(dn00(T1%rt(i, al4), T1%lf(j, al1)), T1%rt(i, al4), T1%lf(j, al1))  &
                            - one_G(dn00(T1%rt(i, al4), T1%dn(j, al1)), T1%rt(i, al4), T1%dn(j, al1))  &
                            - one_G(dn00(T1%up(i, al4), T1%rt(j, al1)), T1%up(i, al4), T1%rt(j, al1))  &
                            + one_G(dn00(T1%up(i, al4), T1%up(j, al1)), T1%up(i, al4), T1%up(j, al1))  &
                            - one_G(dn00(T1%up(i, al4), T1%lf(j, al1)), T1%up(i, al4), T1%lf(j, al1))  &
                            + one_G(dn00(T1%up(i, al4), T1%dn(j, al1)), T1%up(i, al4), T1%dn(j, al1))  &
                            + one_G(dn00(T1%lf(i, al4), T1%rt(j, al1)), T1%lf(i, al4), T1%rt(j, al1))  &
                            - one_G(dn00(T1%lf(i, al4), T1%up(j, al1)), T1%lf(i, al4), T1%up(j, al1))  &
                            + one_G(dn00(T1%lf(i, al4), T1%lf(j, al1)), T1%lf(i, al4), T1%lf(j, al1))  &
                            - one_G(dn00(T1%lf(i, al4), T1%dn(j, al1)), T1%lf(i, al4), T1%dn(j, al1))  &
                            - one_G(dn00(T1%dn(i, al4), T1%rt(j, al1)), T1%dn(i, al4), T1%rt(j, al1))  &
                            + one_G(dn00(T1%dn(i, al4), T1%up(j, al1)), T1%dn(i, al4), T1%up(j, al1))  &
                            - one_G(dn00(T1%dn(i, al4), T1%lf(j, al1)), T1%dn(i, al4), T1%lf(j, al1))  &
                            + one_G(dn00(T1%dn(i, al4), T1%dn(j, al1)), T1%dn(i, al4), T1%dn(j, al1))  
                      ...
                      elseif (count((/al1, al2, al3, al4/) == 1) == 2 .and. count((/al1, al2, al3, al4/) == 2) == 2 .and. mod(al1+al2, 2) == 1) then
                        T1%Pdtau(dt1, T1%tmp, 5) = T1%Pdtau(dt1, T1%tmp, 5) + upt0(ire,jre)*a
                        T1%Pdtau(L, T1%tmp, 5) = T1%Pdtau(L, T1%tmp, 5) + val3 
                      ! P_dppp
                      elseif (count((/al1, al2, al3, al4/) == 1) == 1 .and. count((/al1, al2, al3, al4/) == 2) == 3) then
                        T1%Pdtau(dt1, T1%tmp, 6) = T1%Pdtau(dt1, T1%tmp, 6) + upt0(ire,jre)*a
                        T1%Pdtau(L, T1%tmp, 6) = T1%Pdtau(L, T1%tmp, 6) + val3 
                      ! P_pppp
                      elseif ((al1==2) .and. (al2==2) .and. (al3==2) .and. (al4==2)) then
                        T1%Pdtau(dt1, T1%tmp, 7) = T1%Pdtau(dt1, T1%tmp, 7) + upt0(ire,jre)*a
                        T1%Pdtau(L, T1%tmp, 7) = T1%Pdtau(L, T1%tmp, 7) + val3
                      elseif (count((/al1, al2, al3, al4/) == 2) == 1 .and. count((/al1, al2, al3, al4/) == 3) == 1 .and. &
                                  ((al1+al2)==2 .or. (al3+al4==2))) then
                        T1%Pdtau(dt1, T1%tmp, 8) = T1%Pdtau(dt1, T1%tmp, 8) + upt0(ire,jre)*a
                        T1%Pdtau(L, T1%tmp, 8) = T1%Pdtau(L, T1%tmp, 8) + val3
                      elseif ((al1==3) .and. (al2==3) .and. (al3==3) .and. (al4==3)) then
                        T1%Pdtau(dt1, T1%tmp, 9) = T1%Pdtau(dt1, T1%tmp, 9) + upt0(ire,jre)*a
                        T1%Pdtau(L, T1%tmp, 9) = T1%Pdtau(L, T1%tmp, 9) + val3
      ...
      if (T1%flags(ICOND) == 1) then
        ...
        do i = 1,  T1%properties(ICOND)%n
            do j = 1,  T1%properties(ICOND)%n
              ...
              value1(k) = value1(k) - a* &
                          (-upt0(i,T1%rt(j, 1))*up0t(j,T1%rt(i, 1)) - upt0(T1%rt(i, 1),j)*up0t(T1%rt(j, 1),i) &
                          +upt0(i,j)*up0t(T1%rt(j, 1),T1%rt(i, 1)) + upt0(T1%rt(i, 1),T1%rt(j, 1))*up0t(j,i))
              value1(k) = value1(k) - a* &
                          (uptt(i,T1%rt(i, 1)) - uptt(T1%rt(i, 1),i))*(up00(j,T1%rt(j, 1)) - up00(T1%rt(j, 1),j))*0.5_wp
              ! up*dn terms
              value1(k) = value1(k) - c* &
              ...
              ! value1(k) = value1(k) - a* &
              !             (-upt0(i,T1%up(j, 1))*up0t(j,T1%up(i, 1)) - upt0(T1%up(i, 1),j)*up0t(T1%up(j, 1),i) &
              !               +upt0(i,j)*up0t(T1%up(j, 1),T1%up(i, 1)) + upt0(T1%up(i, 1),T1%up(j, 1))*up0t(j,i))
              ! value1(k) = value1(k) - a* &
              !             ( uptt(i,T1%up(i, 1))*up00(j,T1%up(j, 1)) - uptt(T1%up(i, 1),i)*up00(T1%up(j, 1),j) &
              !               -uptt(i,T1%up(i, 1))*up00(T1%up(j, 1),j) + uptt(T1%up(i, 1),i)*up00(j,T1%up(j, 1)))
              ! up*dn terms
              ! value1(k) = value1(k) - c* &
              !             ( uptt(i,T1%up(i, 1))*dn00(j,T1%up(j, 1)) + uptt(T1%up(i, 1),i)*dn00(T1%up(j, 1),j) &
              !               -uptt(i,T1%up(i, 1))*dn00(T1%up(j, 1),j) - uptt(T1%up(i, 1),i)*dn00(j,T1%up(j, 1)))
            end do
        end do
        ...
        do i = 1,  T1%properties(ICOND)%n
            do j = 1,  T1%properties(ICOND)%n
              ...
              value1(k) = value1(k) - b* &
                          (-dnt0(i,T1%rt(j, 1))*dn0t(j,T1%rt(i, 1)) - dnt0(T1%rt(i, 1),j)*dn0t(T1%rt(j, 1),i) &
                          +dnt0(i,j)*dn0t(T1%rt(j, 1),T1%rt(i, 1)) + dnt0(T1%rt(i, 1),T1%rt(j, 1))*dn0t(j,i))
              value1(k) = value1(k) - b* &
                          (dntt(i,T1%rt(i, 1)) - dntt(T1%rt(i, 1),i))*(dn00(j,T1%rt(j, 1)) - dn00(T1%rt(j, 1),j))*0.5_wp
              ! dn*up terms
              value1(k) = value1(k) - d* &
              ...
              ! value1(k) = value1(k) - b* &
              !             (-dnt0(i,T1%up(j, 1))*dn0t(j,T1%up(i, 1)) - dnt0(T1%up(i, 1),j)*dn0t(T1%up(j, 1),i) &
              !               +dnt0(i,j)*dn0t(T1%up(j, 1),T1%up(i, 1)) + dnt0(T1%up(i, 1),T1%up(j, 1))*dn0t(j,i))
              ! value1(k) = value1(k) - b* &
              !             ( dntt(i,T1%up(i, 1))*dn00(j,T1%up(j, 1)) - dntt(T1%up(i, 1),i)*dn00(T1%up(j, 1),j) &
              !               -dntt(i,T1%up(i, 1))*dn00(T1%up(j, 1),j) + dntt(T1%up(i, 1),i)*dn00(j,T1%up(j, 1)))
              ! dn*up terms
              ! value1(k) = value1(k) - d* &
              !             ( dntt(i,T1%up(i, 1))*up00(j,T1%up(j, 1)) + dntt(T1%up(i, 1),i)*up00(T1%up(j, 1),j) &
              !               -dntt(i,T1%up(i, 1))*up00(T1%up(j, 1),j) - dntt(T1%up(i, 1),i)*up00(j,T1%up(j, 1)))
            end do
        end do
  ...
  subroutine DQMC_TDM_Avg(T1, model)
    ...
    ! ... local scalar ...
    integer  :: nl, idx, i, j, k, it, x1,x2,y1,y2,z1,z2,correction, al1, al2, al3, al4, ire, jre
    integer, intent(in)  :: model
    real(wp) :: factor, fac, tmp, valL , a
    real(kind=wp) :: adw(3, 3, 3, 3)
    ...
    ! IPAIRd%n may cause divided by 0 issue when not to be computed
    fac = factor/(T1%properties(IPAIRd)%n/correction) 
    ...
    if (T1%flags(ISPXX) == 1) then
      do i = 1, T1%properties(ISPXX)%nClass 
        T1%chirxx_iw0(i,idx) = T1%chirxx_iw0(i,idx) * factor
      enddo
    endif

    if (T1%flags(ISPZZ) == 1) then
      do i = 1, T1%properties(ISPZZ)%nClass 
        T1%chirzz_iw0(i,idx) = T1%chirzz_iw0(i,idx) * factor
      enddo
    endif
                ...
                if (mod(i-1,3)==0 .and. mod(j-1,3)==0) then
                  do al2 = 1, 3
                    do al3 = 1, 3
                      ...
                      ire = i+al2-1
                      jre = j+al3-1
                      ! Here tensor P has total 3^4=81 terms 
                      do al1 = 1, 3
                        do al4 = 1, 3
                          adw(al1, al2, al3, al4) = adw(al1, al2, al3, al4)                         &
                                + value2(T1%properties(IPAIRd)%D(T1%rt(i, al4), T1%rt(j, al1))) &
                                - value2(T1%properties(IPAIRd)%D(T1%rt(i, al4), T1%up(j, al1))) &
                                + value2(T1%properties(IPAIRd)%D(T1%rt(i, al4), T1%lf(j, al1))) &
                                - value2(T1%properties(IPAIRd)%D(T1%rt(i, al4), T1%dn(j, al1))) &
                                - value2(T1%properties(IPAIRd)%D(T1%up(i, al4), T1%rt(j, al1))) &
                                + value2(T1%properties(IPAIRd)%D(T1%up(i, al4), T1%up(j, al1))) &
                                - value2(T1%properties(IPAIRd)%D(T1%up(i, al4), T1%lf(j, al1))) &
                                + value2(T1%properties(IPAIRd)%D(T1%up(i, al4), T1%dn(j, al1))) &
                                + value2(T1%properties(IPAIRd)%D(T1%lf(i, al4), T1%rt(j, al1))) &
                                - value2(T1%properties(IPAIRd)%D(T1%lf(i, al4), T1%up(j, al1))) &
                                + value2(T1%properties(IPAIRd)%D(T1%lf(i, al4), T1%lf(j, al1))) &
                                - value2(T1%properties(IPAIRd)%D(T1%lf(i, al4), T1%dn(j, al1))) &
                                - value2(T1%properties(IPAIRd)%D(T1%dn(i, al4), T1%rt(j, al1))) &
                                + value2(T1%properties(IPAIRd)%D(T1%dn(i, al4), T1%up(j, al1))) &
                                - value2(T1%properties(IPAIRd)%D(T1%dn(i, al4), T1%lf(j, al1))) &
                                + value2(T1%properties(IPAIRd)%D(T1%dn(i, al4), T1%dn(j, al1)))
                          ...
                          ! All future quantities calculated directly using binned values
                          ! of (for example G) in DQMC_TDM_Avg need do like this !!!
                          tmp = value1(k)*adw(al1, al2, al3, al4)/(4.d0*T1%properties(IPAIRd)%n/correction)*2.d0*T1%sgn(idx)
                          ...
                          ! P_dddp, only takes orbital d and p_x into account
                          elseif (count((/al1, al2, al3, al4/) == 1) == 3 .and. count((/al1, al2, al3, al4/) == 2) == 1) then
                          ...
                          ! P_dpdp
                          elseif (count((/al1, al2, al3, al4/) == 1) == 2 .and. count((/al1, al2, al3, al4/) == 2) == 2 .and. mod(al1+al2, 2) == 1) then
                            T1%Pd0tau(it, idx, 5) = T1%Pd0tau(it, idx, 5) + tmp
                          ! P_dppp
                          elseif (count((/al1, al2, al3, al4/) == 1) == 1 .and. count((/al1, al2, al3, al4/) == 2) == 3) then
                            T1%Pd0tau(it, idx, 6) = T1%Pd0tau(it, idx, 6) + tmp
                          ! P_pppp
                          elseif ((al1==2) .and. (al2==2) .and. (al3==2) .and. (al4==2)) then
                            T1%Pd0tau(it, idx, 7) = T1%Pd0tau(it, idx, 7) + tmp
                          elseif (count((/al1, al2, al3, al4/) == 2) == 1 .and. count((/al1, al2, al3, al4/) == 3) == 1 .and. &
                                  ((al1+al2)==2 .or. (al3+al4==2))) then
                            T1%Pd0tau(it, idx, 8) = T1%Pd0tau(it, idx, 8) + tmp
                          elseif ((al1==3) .and. (al2==3) .and. (al3==3) .and. (al4==3)) then
                            T1%Pd0tau(it, idx, 9) = T1%Pd0tau(it, idx, 9) + tmp  
                          endif
...
#   endif
    ...
    if (nproc .eq. 1) then
       ...
       if (T1%flags(ISPXX) == 1) then
         do i = 1, T1%properties(ISPXX)%nClass
           data =  T1%chirxx_iw0(i,1:n)
           call DQMC_SignJackKnife(n, average, error, data, y, sgn, sum_sgn)
           T1%chirxx_iw0(i,avg) = average
           T1%chirxx_iw0(i,err) = error
         enddo
       endif
       ...
       if (T1%flags(ISPZZ) == 1) then
         do i = 1, T1%properties(ISPZZ)%nClass
           data =  T1%chirzz_iw0(i,1:n)
           call DQMC_SignJackKnife(n, average, error, data, y, sgn, sum_sgn)
           T1%chirzz_iw0(i,avg) = average
           T1%chirzz_iw0(i,err) = error
         enddo
       endif
...
#      ifdef _QMC_MPI
          ...
          if (T1%flags(ISPXX) == 1) then
            ...
            bins => T1%chirxx_iw0(:,1)
            aves => T1%chirxx_iw0(:,avg)
          ...
          if (T1%flags(ISPZZ) == 1) then
            ...
            bins => T1%chirzz_iw0(:,1)
            aves => T1%chirzz_iw0(:,avg)
            call mpi_allreduce(bins, aves, n, mpi_double, &
               mpi_sum, mpi_comm_world, mpi_err)
          endif
...
#      endif
  ...
  subroutine DQMC_TDM_Print(T1, ofile, OPT, OPT1, OPT2)
    ...
    if (T1%flagsFT(ISPXX)==1 .and. T1%flagsFT(ISPZZ)==1) then
      ...
      do b1 = 1, T1%norb
        do b2 = 1, T1%norb
          idx = (b1-1)*T1%norb+b2
          write(OPT1,'(2(i4),4(e16.8))') b1-1, b2-1, &
             T1%chiqxx_iw0(1,idx,T1%avg), T1%chiqxx_iw0(1,idx,T1%err), &
             T1%chiqzz_iw0(1,idx,T1%avg), T1%chiqzz_iw0(1,idx,T1%err)
        enddo
      enddo
      ...
      do b1 = 1, T1%norb
        do b2 = 1, T1%norb
          idx = (b1-1)*T1%norb+b2
          write(OPT1,'(2(i4),4(e16.8))') b1-1, b2-1, &
             T1%chiqxx_iw0(2,idx,T1%avg), T1%chiqxx_iw0(2,idx,T1%err), &
             T1%chiqzz_iw0(2,idx,T1%avg), T1%chiqzz_iw0(2,idx,T1%err)
        enddo
      enddo
      ...
      do t = 0, T1%L-1
        write(OPT2,'(f10.5,8(f16.8))') t*T1%dtau, &
             T1%chiqxx(t,1,T1%norb2+1,T1%avg), T1%chiqxx(t,1,T1%norb2+1,T1%err), &
             T1%chiqzz(t,1,T1%norb2+1,T1%avg), T1%chiqzz(t,1,T1%norb2+1,T1%err), &
             T1%chiqxx(t,2,T1%norb2+1,T1%avg), T1%chiqxx(t,2,T1%norb2+1,T1%err), &
             T1%chiqzz(t,2,T1%norb2+1,T1%avg), T1%chiqzz(t,2,T1%norb2+1,T1%err)
      enddo
  ...
  subroutine DQMC_TDM_Print_local(T1, ofile, OPT1, OPT2, OPT3, OPT4, OPT5)
    ...
    if (T1%flags(ISPXX)==1 .and. T1%flags(ISPZZ)==1) then
      call DQMC_open_file('chi_r0_iw0_'//adjustl(trim(ofile)),'replace', OPT5)
      ...
      do i = 1, T1%properties(ISPXX)%nclass
        ...
        if (abs(x)<1.e-5 .and. abs(y)<1.e-5) then
          write(OPT5,'(2(i4),4(e16.8))') b1,b2, &
                 T1%chirxx_iw0(i,T1%avg), T1%chirxx_iw0(i,T1%err), &
                 T1%chirzz_iw0(i,T1%avg), T1%chirzz_iw0(i,T1%err)
        endif
      enddo
    endif
  ...
  subroutine DQMC_TDM_GetErrKFTold(T1)
    ...
    complex(wp), allocatable :: avg(:)
    allocate(avg(size(T1%properties(1)%valueskold(:,1,1))))
      ...
      do ip = 1, NTDMARRAY-1 ! -1 for exclusion of conductivity for FT
        if (T1%flags(ip)==1) then
          ...
          do it = 0, T1%L-1
             ...
             !write(*, *) "The value of average is: ", average
             !avg(:) = cmplx(0.0_wp, 0.0_wp, kind=wp)

             ! Calculate the average of all binval
             !do i = 1, T1%nbin
             !    binval => T1%properties(ip)%valueskold(:,it,i)
             !    !write(*, *) "The value of avg is: ", avg
             !    avg(:) = avg(:) + binval(:)  ! Accumulate all binval
             !enddo
             !avg = avg / dble(T1%nbin)  ! Compute the average
             
             ! Assign the new avg to the location pointed by average
             !average(:) = avg(:)  ! Copy avg to average
             !write(*, *) "The value of avg_new is: ", average
             !write(*, *) "The value of error is: ", error

             do i = 1, T1%nbin
                binval => T1%properties(ip)%valueskold(:,it,i)
                !write(*, *) "The value of binval is: ", binval
